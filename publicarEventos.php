<?php
    ...
    include_once("LeerEventos-sql.php");
				...
				<table class="table table-striped">
                    ...
                    <?php while ($fils=$sql->fetch(PDO::FETCH_ASSOC)){?>
                           <tr>
                                ...
                                <td><?php echo $fils['publicarsino'] = ($fils['publicarsino']==1) ? 'Si':'No'  ?></td>
								<td><?php echo $row['']; ?></td>
								<td><a href="modificar.php?id=<?php echo $row['id']; ?>"><span class="glyphicon glyphicon-pencil"></span></a></td>
                               <td><a href="confirmarPublicarEvento.php?id=<?php echo $fils['id']; ?>" class="btn btn-success btn-sm rounded-pill py-0 editLink"  data-target="#publicar">Publicar</a></td>
								      <td><a onclick="eliminar_fila(<?php echo $fils['id']; ?>)" data-href="eliminar.php?id=<?php echo $row['id']; ?>" class="btn btn-danger btn-sm rounded-pill py-0 editLink"data-toggle="modal" data-target="#confirm-delete">Dar Baja Al Evento</a></td>
							</tr>
						<?php }?>
					</tbody>
				</table>
